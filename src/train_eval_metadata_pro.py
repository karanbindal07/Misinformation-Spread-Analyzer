#!/usr/bin/env python3
"""Train and evaluate a metadata-only PolitiFact baseline with deployment-safe evaluation."""
...
def load_metadata_dataset(real_path: str, fake_path: str) -> pd.DataFrame:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train/evaluate metadata-only professional baseline")
...
def main() -> None:
    ...
    df = load_metadata_dataset(args.real_path, args.fake_path)
    split = split_train_val_test(df=df, test_size=0.2, val_size=0.2, random_state=args.random_state)

    feat_cols = ["tweet_count", "has_tweets", "log_tweet_count", "title_len", "url_len"]
    ...
    X_val = scaler.transform(val_df[feat_cols])
    ...
    scaler_path = os.path.join(args.models_dir, "metadata_scaler_pro.joblib")
    model_path = os.path.join(args.models_dir, "metadata_lr_model_pro.joblib")
    manifest_path = os.path.join(args.models_dir, "model_manifest_metadata_pro.json")
    val_table_path = os.path.join(args.outputs_dir, "metadata_validation_threshold_table.csv")
    test_metrics_path = os.path.join(args.outputs_dir, "metadata_test_metrics.json")
